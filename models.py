...
class LogNormalDataset(Dataset):
    def __init__(self, percorso_x, percorso_y, percorso_pk):
        ...
        self.media_y = self.labels.mean(
            dim=0
        )  # dim=0 per normalizzare i tre parametri singolarmente, colonna per colonna
...
def vgg_block(num_convs, in_channels, out_channels):
    ...
    for _ in range(num_convs):
        ...
        layers.append(nn.ReLU())  # activ per mantenere anche underdensity
    ...
    def __init__(self, conv_arch=None):
        ...
        self.head = nn.Sequential(
            nn.AdaptiveAvgPool2d(
                (1, 1)
            ),  # dovrebbe diminuire overfitting e imparare concetti globali; permette anche di accettare qualsiasi dimensione iniziale
...
class InceptionBlock(nn.Module):
    ...
    ):  # c sono canali in uscita di ogni ramo
        super(InceptionBlock, self).__init__()

        # RAMO 1: 1x1
        self.ramo1 = nn.Sequential(
            nn.Conv2d(in_channels, c1, kernel_size=1), nn.BatchNorm2d(c1), nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        return torch.cat(
            (out1, out2, out3, out4), dim=1
        )  # concatena le feature lungo la dimensione dei canali (dim=1)
...
class CosmoGoogLeNet(nn.Module):
    def __init__(self):
        ...
        self.pool_mezzo = nn.MaxPool2d(
            kernel_size=3, stride=2, padding=1
        )  # -> # res_finale = res_iniziale/8
...
class ResidualBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_1x1conv=False, strides=1):
        ...
        self.conv1 = nn.Conv2d(
            in_channels,
            out_channels,
            kernel_size=3,  # questa conv può cambiare res
            padding=1,
            stride=strides,
        )
        ...
        )  # questa mantiene sempre stessa res
    ...
    def forward(self, X):
        Y = self.activ(self.bn1(self.conv1(X)))
...
def crea_fase_resnet(in_channels, out_channels, num_blocks, prima_fase=False):
    ...
    for i in range(num_blocks):
        if i == 0 and not prima_fase:
            blocks.append(
                ResidualBlock(in_channels, out_channels, use_1x1conv=True, strides=2)
            )
        elif i == 0 and prima_fase and in_channels != out_channels:
            blocks.append(
                ResidualBlock(in_channels, out_channels, use_1x1conv=True, strides=1)
            )
        else:
...
class CosmoResNet(nn.Module):
    def __init__(
        self, architettura=((3, 64), (4, 128), (6, 256), (3, 512))
    ):  # ResNet-18 (2, 64), (2, 128), (2, 256), (2, 512)
        ...
        for i, (num_blocks, out_channels) in enumerate(architettura):
            ...
            fase = crea_fase_resnet(
                in_channels, out_channels, num_blocks, prima_fase=is_prima_fase
            )
        ...
        self.head = nn.Sequential(
            ...
            nn.ELU(),
            nn.Linear(128, 1),  # output = (omega_m, n_s, sigma_8)
        )
...
class BottleneckBlock(nn.Module):
    def __init__(self, in_channels, out_channels, use_1x1conv=False, strides=1):
        ...
        # In un bottleneck, i canali intermedi sono sempre 1/4 dell'output finale
        ...
        self.conv2 = nn.Conv2d(
            mid_channels, mid_channels, kernel_size=3, padding=1, stride=strides
        )
    ...
    def forward(self, X):
        Y = self.activ(self.bn1(self.conv1(X)))
        Y = self.activ(self.bn2(self.conv2(Y)))
...
def crea_fase_resnet_bottleneck(
    in_channels, out_channels, num_blocks, prima_fase=False
):
    ...
    for i in range(num_blocks):
        if i == 0 and not prima_fase:
            blocks.append(
                BottleneckBlock(in_channels, out_channels, use_1x1conv=True, strides=2)
            )
        elif i == 0 and prima_fase and in_channels != out_channels:
            blocks.append(
                BottleneckBlock(in_channels, out_channels, use_1x1conv=True, strides=1)
            )
        else:
            blocks.append(BottleneckBlock(out_channels, out_channels))

    return nn.Sequential(*blocks)


class CosmoResNet50(nn.Module):
    def __init__(self, architettura=((3, 256), (4, 512), (6, 1024), (3, 2048))):
        super(CosmoResNet50, self).__init__()
        ...
        for i, (num_blocks, out_channels) in enumerate(architettura):
            ...
            fase = crea_fase_resnet_bottleneck(
                in_channels, out_channels, num_blocks, prima_fase=is_prima_fase
            )
...
class CosmoResNet3head(nn.Module):
    def __init__(
        self, architettura=((2, 64), (2, 128), (2, 256), (2, 512))
    ):  # ResNet-18
        ...
        self.head_om = nn.Sequential(
            nn.Linear(in_channels, 64), nn.ReLU(), nn.Linear(64, 1)
        )
...
class CosmoResNetNLL(nn.Module):
    def __init__(
        self, architettura=((2, 64), (2, 128), (2, 256), (2, 512))
    ):  # ResNet-18      (3, 64), (4, 128), (6, 256), (3, 512)
        ...
        self.head_om = nn.Sequential(
            nn.Linear(in_channels, 64), nn.ReLU(), nn.Linear(64, 2)
        )
...
class IbridaCosmoResNet50(nn.Module):
    def __init__(
        self, pk_length, architettura=((3, 256), (4, 512), (6, 1024), (3, 2048))
    ):
        super(IbridaCosmoResNet50, self).__init__()
