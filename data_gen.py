...
COSMOLGIE_NUM = 200  # Punti nel LH
UNIVERSI_NUM = 20000
RESOLUTION = 512
...
def genera_pk_cosmologico(omega_m, n_s, sigma8):
    ...
    )  # non linearità data da trasf. log-normal (paper Coles e Jones)

    params.set_for_lmax(
        2500, lens_potential_accuracy=0
    )  # 2500 è quello di Planck, serve per inzializzare
    # accuratezza 0 perché non mi intressa davvero calcolarlo

    results = camb.get_results(params)

    sigma8_default = results.get_sigma8()[
        0
    ]  # vedo quale sigma_8 è stato generato casualmente
    sigma8_factor = (sigma8 / sigma8_default) ** 2  # fattore correzione a posteriori

    kh, z, pk = results.get_matter_power_spectrum(
        minkh=1e-4, maxkh=10, npoints=500
    )  # array dei k, dei redshift, del pk per ogni k (sarebbe matrice ma c'è solo un redshift)
    ...
    pk_interp = interp1d(
        k_mpc, pk_2d, kind="cubic", fill_value="extrapolate"
    )  # fit con scipy.interpolate
...
def main():
    ...
    campioni_lhs = qmc.scale(
        campionatore.random(n=COSMOLGIE_NUM), LIMITI_INF, LIMITI_SUP
    )  # matrice in cui ogni riga è una cosmologia definita dai parametri nelle 3 colonne
    ...
    tensore_x_finale = torch.empty((UNIVERSI_NUM, 1, 512, 512), dtype=torch.float32)
    tensore_y_finale = torch.empty((UNIVERSI_NUM, 3), dtype=torch.float32)
    tensore_pk_finale = torch.empty((UNIVERSI_NUM, 232), dtype=torch.float32)
    ...
    for i in tqdm(range(UNIVERSI_NUM), desc="Generazione Powerbox"):
        ...
        pb = pbox.PowerBox(N=RESOLUTION, dim=2, pk=pk_chosen, boxlength=BOX_LENGTH)
        delta_g = (
            pb.delta_x()
        )  # genera campo gaussiano usando powerbox appena configurato sopra

        varianza = np.var(delta_g)
        delta_ln = (
            np.exp(delta_g - varianza / 2.0) - 1.0
        )  # genera campo ln assicurando che abbia media 0 e che non ci siano NaN o inf (derivazione formula quad potsdam)

        P_k_1D, k_bins, *extra = pbox.get_power(delta_ln, boxlength=BOX_LENGTH)
        mask = k_bins > 0
        P_k_1D_pulito = P_k_1D[mask] * BOX_LENGTH

        tensore_x = torch.tensor(delta_ln, dtype=torch.float32).unsqueeze(
            0
        )  # unsqueeze per aggiungere canale (1,RES,RES)
        tensore_y = torch.tensor(label_chosen, dtype=torch.float32)
        tensore_pk = torch.tensor(P_k_1D_pulito, dtype=torch.float32)

        tensore_x_finale[i] = tensore_x
        tensore_y_finale[i] = tensore_y
        tensore_pk_finale[i] = tensore_pk
