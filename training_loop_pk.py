...
from modelCNN import LogNormalDataset, IbridaCosmoResNet, SimpleIbridaCosmoNet

def aggiorna_grafico_loss(storico_train, storico_val, percorso_plot):
    ...
    epoca_migliore = storico_val.index(min(storico_val))
    plt.scatter(epoca_migliore, min(storico_val), color='green', s=100, zorder=5, label='Best Model')
...
def main():
    ...
    generatore = torch.Generator().manual_seed(42)  # seed manuale per avere sempre stessa divisione 
    dati_train, dati_val, dati_test = random_split(
        dataset_completo, 
        [dim_train, dim_val, dim_test], 
    ...
    modello = SimpleIbridaCosmoNet(pk_length=lunghezza_pk).to(device)
    ...
    if os.path.exists(PERCORSO_MODELLO):
        ...
        modello.load_state_dict(torch.load(PERCORSO_MODELLO, map_location=device, weights_only=True))
    ...
    miglior_loss_val = float('inf') # serve per trovare il minimo della loss iterativamente 
    ...
    for epoca in range(EPOCHE):
        ...
        for batch_x, batch_pk, batch_y in train_loader:
            batch_x, batch_pk, batch_y = batch_x.to(device), batch_pk.to(device), batch_y.to(device)
            ...
            # Gradient Clipping
            torch.nn.utils.clip_grad_norm_(modello.parameters(), max_norm=1.0)  # max_norm è la massima norma L2 di tutti i gradienti sommati
            ottimizzatore.step()
            ...
            with torch.no_grad():
                ss_res_train += torch.sum((batch_y - previsioni) ** 2).item()                   # discostamento previsioni da labels
                ss_tot_train += torch.sum((batch_y - torch.mean(batch_y, dim=0)) ** 2).item()   # varianza dei labels
            
        media_loss_train = loss_train_totale / dim_train
        acc_train = max(0.0, 1 - (ss_res_train / (ss_tot_train + 1e-8))) # taglia a 0 se negativo
        ...
        with torch.no_grad():
            for batch_x, batch_pk, batch_y in val_loader:
                ...
                ss_tot_val += torch.sum((batch_y - torch.mean(batch_y, dim=0)) ** 2).item()
                
        media_loss_val = loss_val_totale / dim_val
        acc_val = max(0.0, 1 - (ss_res_val / (ss_tot_val + 1e-8)))
        storia_val_loss.append(media_loss_val)
        
        aggiorna_grafico_loss(storia_train_loss, storia_val_loss, PERCORSO_GRAFICO_LOSS)
